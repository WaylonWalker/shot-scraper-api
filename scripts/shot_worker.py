...
class Screenshot(BaseModel):
    ...
    @classmethod
    def validate_scaled_dimensions(
        cls, v: Optional[int], info: ValidationInfo
    ) -> Optional[int]:
        ...
        scaled_width = (
            v if info.field_name == "scaled_width" else values.get("scaled_width")
        )
        scaled_height = (
            v if info.field_name == "scaled_height" else values.get("scaled_height")
        )
...
def process_image(screenshot: Screenshot) -> bool:
    ...
    try:
        ...
        if screenshot.needs_scaling:
            typer.echo(
                f"Resizing to {screenshot.scaled_width}x{screenshot.scaled_height}"
            )
            ...
            resize_cmd = [
                ...
                "-background",
                "none",  # Transparent background for padding
                screenshot.output,
            ]
    ...
    with ThreadPoolExecutor(max_workers=min(32, len(screenshots))) as executor:
        ...
        try:
            ...
            captured = await asyncio.gather(
                *[sem_task(screenshot) for screenshot in screenshots]
            )
        finally:
            try:
                await browser.close()
...
async def process_queue(queue):
    ...
    for filename, item in queue.items():
        try:
            ...
            if screenshot.needs_scaling:
                typer.echo(
                    f"Will scale: {screenshot.width}x{screenshot.height} -> {screenshot.scaled_width}x{screenshot.scaled_height}"
                )
        except Exception as e:
    ...
    # asyncio.run(take_screenshots(screenshots))
    await take_screenshots(screenshots)
...
async def main():
    ...
    while True:
        ...
        # queue = await redis.brpop("shot-queue")
        _, raw = await redis.blpop("job_queue", timeout=0)

        queue = json.loads(raw)
        queue = {queue["output_final"]: queue}
        print(f"Queue: {queue}")
        if queue:
